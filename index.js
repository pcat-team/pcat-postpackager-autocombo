...
function createRequireConfig(ret, conf, settings, opt) {
    ...
    const inlineFirstScreenCss = settings.inlineFirstScreenCss
    ...
    // 将首屏（isFirstScreen）模块的依赖也加上 isFirstScreen 标示

    function addDepsIsFirstScreen(deps) {
        deps.forEach((file, index) => {
            if (res[file].extras) {
                res[file].extras.isFirstScreen = true;
            } else {
                res[file].extras = {
                    isFirstScreen: true
                }
            ...
            if (res[file].deps) {
                addDepsIsFirstScreen(res[file].deps);
            }
        })
    }

    files.forEach(function(_file, i) {
        const extras = res[_file].extras;
        if (extras && extras.isFirstScreen) {

            const deps = res[_file].deps || [];
            addDepsIsFirstScreen(deps);
        }
    ...
    files.forEach(function(_file) {
        ...
        oneDeps.length > 0 && ! function() {
            ...
            _copy.map(function(__file) {
                ...
                let isFirstScreen = (res[__file] && res[__file].extras) ? res[__file].extras.isFirstScreen : false;
        ...
        if (file && (file._likes.isHtmlLike || file._likes.isJsLike)) {
            ...
            Object.keys(comboCSS).map(function(type) {
                ...
                if (type == 88888888) {
                    ...
                    const inline = inlineFirstScreenCss ? '__inline="true"' : '';
                    ...
                    if (oConfig.combo) {
                        ...
                        firstScreen.push('<link ' + inline + ' href="' + DOMAIN + COMBO_URL + urls.join(',') + '" type="text/css" rel="stylesheet" first="true" />')

                    } else {
                        firstScreen.push(thisComboCSS.map(function(obj) {
                            return '<link ' + inline + ' href="' + DOMAIN + obj.url + '" type="text/css" rel="stylesheet" first="true" />'
                        }).join('\n'))
                    }
            ...
            file.setContent(content.replace(/<!--REQUIRE_CONFIG-->|__REQUIRE_CONFIG__|window\.__REQUIRE_CONFIG__/g, config)
                ...
                .replace('<!--FIRSTSCREEN-->', firstScreen.join('\n'))
